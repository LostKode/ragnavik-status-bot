...
id -u ragnavik-status >/dev/null 2>&1 || useradd --system --home /nonexistent --shell /usr/sbin/nologin ragnavik-status
install -d -m 0700 -o ragnavik-status -g ragnavik-status /etc/ragnavik-status
...
for token in hook control; do
  ...
  chown ragnavik-status:ragnavik-status "$file"
  chmod 0600 "$file"
done
